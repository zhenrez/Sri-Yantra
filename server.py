...
import json, sqlite3, argparse, uuid
...
def init():
    with connection() as db:
        ...
        CREATE TABLE IF NOT EXISTS relation_events(id TEXT PRIMARY KEY,overlay_id TEXT NOT NULL REFERENCES overlays(id),kind TEXT NOT NULL,feature_id TEXT,reason TEXT NOT NULL DEFAULT '');
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def do_GET(self):
        if self.path=='/api/state':
            with connection() as db:
                self.send_json(200,{k:[dict(row) for row in db.execute('SELECT * FROM '+k)] for k in ('overlays','entities','placements','relation_events','event_participants')})
    ...
    def do_POST(self):
        ...
        with connection() as db:
            ...
            if self.path=='/api/placements':
                ...
                status=p.get('status','exploratory');role=p.get('role','candidate')
                ...
                pid=str(uuid.uuid4());db.execute('INSERT INTO placements(id,overlay_id,entity_id,profile,feature_id,role,status,source,reason,confidence) VALUES (?,?,?,?,?,?,?,?,?,?)',(pid,p['overlay'],eid,profile,feature,role,status,str(p.get('source',''))[:1000],str(p.get('reason',''))[:3000],None));self.send_json(201,{'id':pid})
            else:
                ...
                event=str(uuid.uuid4());db.execute('INSERT INTO relation_events VALUES (?,?,?,?,?)',(event,p['overlay'],'proposed_generation',feature,str(p.get('reason',''))[:3000]))
